...
export default {
  ...
  computed: {
    ...
    marketAddress() {
      switch (this.wallet.network) {
        ...
        case 'rinkeby':
          return process.env.VUE_APP_THIRDWEB_RINKEBY
        case 'goerli':
          return process.env.VUE_APP_THIRDWEB_GOERLI
      }
    },
  ...
  methods: {
    async init() {
      ...
      const normalize = nft => !nft ? null : ({ owner: nft.owner, ...nft.metadata });
      [ this.claimedNFT, this.nfts ] = await Promise.all([
        this.drop.getOwned(this.address).then(_.first).then(normalize),
        this.drop.getAll().then(nfts => _.map(nfts, normalize))
      ]);
    },
    async scanForMoves() {
      const { lobby } = this.contracts;
      const eventFilter = lobby.filters.PlayerMoved(null, this.address);
      const moves = await lobby.queryFilter(eventFilter);
      return moves;
    },
    async claim(id) {
    ...
    sample() { return _.sample(this.claimed, 8) },
  ...
  <div id='marketplace'>
    ...
    <div class='flex'>
      <div class='flex-shrink padded bordered container margin'>
        <div class='text-center text-lg bold'>{{ displayNFT.name }}</div>
      ...
      <div class='flex-1 margin-lg'>
        ...
        <div v-else>
          <div>
            Thank you for visiting The Blockchain Chess Lounge, a competitive chess platform for Ethereum.  The players of the first 100 games will get an opportunity to claim one Founders Edition NFT.  Simply start a game and submit one move to qualify for the drop.
          </div>
          <div class='margin-lg-tb' />
          <div>
            As a security precaution, wagering has been temporarily disabled on Mainnet.  If you'd like to help with testing, simply play on one of our supported test networks and file bugs on github.  You may qualify for airdrops or other cool prizes for pitching in.
          </div>
          <div class='margin-lg-tb' />
          <div>
            Our goal is to host a Christmas Chess Tournament for NFTs and crypto prizes.  Please consider supporting our work.
          </div>
        </div>
      </div>
    </div>
    ...
    <div class='flex flex-wrap'>
      <div v-for='nft in sample()' class='padded bordered container margin'>
        <div class='text-center bold'>{{ nft.name }}</div>
